...
################## Process Function ##################	
    ...
    def set_file_photo_name(self, ext='png'):
        path = '/home/'+username+'/Pictures/C922/VID_'
    ...
    def set_file_video_name(self, ext='avi'):
        path = '/home/'+username+'/Videos/C922/VID_'
        time = str(strftime("%Y-%b-%d at %H.%M.%S %p"))
        ext = ext
    ...
    def take_photo(self):
        ...
        while stream.isOpened():
            ...
            cv2.imwrite(pathfile.format(count), frame)
    ...
    def std_dimensions(self):
        ...
        "16k"  : (15360, 8640),
        "64k"  : (61440, 34560)}
        return STD_DIMENSIONS

    def change_res(self, stream, width, height):
        stream.set(3, width)
        stream.set(4, height)

    # grab resolution dimensions and set video capture to it.
    def get_dims(self, stream, res='720p'):
        width, height = self.std_dimensions["720p"]
        if res in self.std_dimensions:
            width,height = self.std_dimensions[res]
        self.change_res(stream, width, height)
        return width, height
    ...
    def put_text_on_video(self, frame, fps, time):
    ...
    def record_video(self, stream, frames_per_second=30):
        ...
        filename = self.set_file_video_name()
        # out = cv2.VideoWriter(filename[0], filename[1], frames_per_second, self.get_dims(stream, resolution))
        # print(filename)
        # width = int(stream.get(cv2.CAP_PROP_FRAME_WIDTH))
        # height = int(stream.get(cv2.CAP_PROP_FRAME_HEIGHT))
        out = cv2.VideoWriter(filename[0], cv2.VideoWriter_fourcc(*'DIVX'), frames_per_second, (width, height))
        ...
        while stream.isOpened():
            ...
            frame = cv2.flip(frame, 1)
            # puting the FPS count on the frame and display date and time
            self.put_text_on_video(frame, display_fps, time)
            ...
            if key == ord('q'):
                while j>=10:
                    ...
                    # puting the FPS count on the frame and display date and time
                    self.put_text_on_video(frame, fps, time)
    ...
    def get_fps(self):
        # print("Get resolution from resolution combobox")
        fps = self.cbb_video_framerate.currentText()
        return fps
